    ...
    public void doAECM() {
        try {
            ...
            for (/* empty */; fin.read(pcmInputCache, 0, pcmInputCache.length) != -1; /* empty */) {
                ...
                // aecm procession, for now the echo tail is hard-coded 100ms, but you
                // should estimate it correctly each time you call echoCancellation, otherwise aecm
                // cannot work.
                aecm.farendBuffer(aecTmpIn, cacheSize / 2);
                aecm.echoCancellation(aecTmpIn, null, aecTmpOut,
                        (short) (cacheSize / 2), (short) 100);
